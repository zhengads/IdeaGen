...
class ModelFactory:
    ...
    def create_model_for_agent(agent_type: str, config: Dict[str, Any]) -> BaseModel:
        ...
        # Apply agent-specific overrides from config
        for key in ["temperature", "max_tokens", "model_name", "fallbacks"]:
            if key in config:
                model_config[key] = config[key]
        
        # Check environment variables for agent-specific models (.env override)
        import os
        env_key = f"{agent_type.upper()}_MODELS"
        env_models = os.environ.get(env_key)
        if env_models:
            # Parse comma-separated list
            model_list = [m.strip() for m in env_models.split(",") if m.strip()]
            if model_list:
                model_config["model_name"] = model_list[0]
                if len(model_list) > 1:
                    model_config["fallbacks"] = model_list[1:]
