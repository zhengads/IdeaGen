...
class BaseAgent(abc.ABC):
    ...
    async def _call_model(self,
        ...
        backoff_delay = 1.0  # Start with 1 second, doubles each retry
        
        while True:
            try:
                logger.info(f"Agent {self.name} calling model ({self.model.model_name})...")
                ...
                logger.info(f"Agent {self.name} model call completed in {duration:.2f}s")
                return response
                    
            except Exception as e:
                remaining_retries -= 1
                logger.warning(f"Agent {self.name} model call failed: {str(e)}. "
                             f"Retries left: {remaining_retries}, backoff: {backoff_delay:.1f}s")
